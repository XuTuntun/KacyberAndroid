...
import com.kacyber.base.CirculateAction;
import com.kacyber.network.http.HttpClientHelper;
import com.kacyber.network.http.HttpReqResult;
...
public class HttpAsyncTaskServices extends AsyncTask<Void, Void, HttpReqResult> {
	...
	protected HttpReqResult doInBackground(Void... paramTemp) {
		...
		try {
			if (action.getType() == BaseAction.ActionType.Normal) {
				result = HttpClientHelper.executePost(action.getRequestUrl().trim(), action.getHttpEntity());
			} else if (action.getType() == BaseAction.ActionType.Circulate) {
				...
				for (int i = 0, n = httpEntityList.size(); i < n; i++) {
					...
					if (HttpReqResult.ResultType.SUCCESS == result.getResult()) {
